...
@transaction.atomic
def save_external_book(book_data: dict[str, Any]) -> Book | None:
    """
    ...
    try:
                ...
                try:
                    if genre_name and isinstance(genre_name, str):
                        ...
                        primary_genre, created = Genre.objects.get_or_create(
                            ...
                            defaults={"name": genre_name, "description": f"Books in the {genre_name} category"},
                        )

                        # Handle genre relationships and hierarchies
                        if created:
                            # For new genres, try to find related genres
                            related_genres = Genre.objects.filter(name__icontains=genre_name).exclude(
                                id=primary_genre.id
                            )
                            ...
                            if related_genres.exists():
                                ...
                                if len(most_specific.name) > len(genre_name):
                                    ...
                                    logger.info(
                                        f"Using more specific genre '{most_specific.name}' instead of '{genre_name}'"
                                    )
                                else:
                                    ...
                                    primary_genre.description = (
                                        f"Books in the {genre_name} category. Related to: {', '.join(related_names)}"
                                    )
                                    primary_genre.save()
        ...
        if not genres_added:
            default_genre, _ = Genre.objects.get_or_create(
                name="Uncategorized",
                defaults={"name": "Uncategorized", "description": "Books that haven't been categorized yet"},
            )
            try:
