        ...
        try:
            ...
            if isbn13 and len(isbn13) == 13:
                response = session.get(
                    cls.SEARCH_URL,
                    params={"q": f"isbn:{isbn13}", "limit": page_size, "fields": ",".join(cls.FIELDS)},
                    timeout=(5, 10),
                )
            else:
                response = session.get(
                    cls.SEARCH_URL,
                    params={"q": query, "limit": page_size, "fields": ",".join(cls.FIELDS)},
                    timeout=(5, 10),
                )
    ...
    def search_books(cls, query: str, page_size: int = 15) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if isbn13 and len(isbn13) == 13:
                ...
                response = session.get(
                    cls.BASE_URL,
                    params={"q": query, "maxResults": min(page_size, cls.MAX_RESULTS), "fields": cls.FIELDS},
                    timeout=(5, 10),
                )
            ...
            for item in data.get("items", []):
                ...
                isbn13 = clean_isbn(
                    next(
                        (
                            id_info.get("identifier")
                            for id_info in industry_identifiers
...
def evaluate_book_completeness(book: dict[str, Any]) -> float:
    ...
    for field, weight in required_fields.items():
        if field in book and book[field] and (field not in {"authors", "genres"} or len(book[field]) > 0):
            score += weight
...
def merge_book_results(
    openlibrary_books: list[dict[str, Any]], googlebooks_books: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    """
    ...
    best_books: dict[str, dict[str, Any]] = {}
    ...
    for book in openlibrary_books:
        ...
        if isbn13 not in best_books or score > best_books[isbn13]["score"]:
            best_books[isbn13] = {"book": book, "score": score, "source": "openlibrary"}
    ...
    sorted_books = sorted(best_books.values(), key=lambda x: float(x["score"]), reverse=True)
    return [cast("dict[str, Any]", info["book"]) for info in sorted_books]
