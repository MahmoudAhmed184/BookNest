...
class BookNormalizer:
    ...
    def normalize(cls, book_data: dict[str, Any], source: str) -> dict[str, Any]:
        ...
        author_names = (
            ", ".join([author["name"] if isinstance(author, dict) else author for author in normalized["authors"]])
            or cls.DEFAULT_AUTHOR
        )
        genre_names = ", ".join(normalized["genres"]) or cls.DEFAULT_GENRE

        logger.info(
            f"Normalized Book: '{normalized['title']}' by {author_names} from {source} with genres: {genre_names}"
        )

        return normalized

    @classmethod
    def _normalize_isbn13(cls, isbn13) -> str | None:
    ...
    @classmethod
    def _normalize_isbn(cls, isbn) -> str | None:
    ...
    @classmethod
    def _normalize_publication_date(cls, pub_date) -> date | None:
        """Normalize publication date"""
        if not pub_date:
            return None
    ...
    @classmethod
    def _normalize_number_of_pages(cls, pages) -> int | None:
        """Normalize number of pages"""
        if not pages:
            return None
