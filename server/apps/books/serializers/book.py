...
from apps.books.models import Author, Book, BookAuthor, Genre, ReadingList
...
class BookSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data: dict[str, Any]) -> Book:
        authors_data = validated_data.pop(
            "authors"
        )  # pop authors data from validated data to use it in creating book authors through BookAuthor model
        genres_data = validated_data.pop(
            "genres", []
        )  # pop genres data from validated data to use it in creating book genres through BookGenre model
        book = Book.objects.create(
            **validated_data
        )  # create the book instance with the rest of the validated data fields

        for author_data in authors_data:
            author, _created = Author.objects.get_or_create(
                **author_data
            )  # get or create author instance from the author data
            BookAuthor.objects.create(
                book=book, author=author
            )  # create the book author instance with the book and author instances

        for genre_name in genres_data:
            genre, _created = Genre.objects.get_or_create(name=genre_name)
...
class ReadingListSerializer(serializers.ModelSerializer):
    ...
    def get_book_count(self, obj: ReadingList) -> int:
        return obj.books.count()

    def get_owner_username(self, obj: ReadingList) -> str | None:
