...
def sync_after_author_link_change(sender, instance: BookAuthor, **_kwargs) -> None:
    del sender
...
def sync_after_genre_link_change(sender, instance: BookGenre, **_kwargs) -> None:
    del sender
    _sync_book_after_commit(instance.book_id)
...
def sync_after_author_save(sender, instance: Author, **_kwargs) -> None:
    del sender
...
def sync_after_genre_save(sender, instance: Genre, **_kwargs) -> None:
    del sender
    for book in instance.books.all():
        transaction.on_commit(lambda book=book: sync_book_denormalized_labels(book=book))
...
def sync_author_likes(sender, instance: AuthorLike, **_kwargs) -> None:
    del sender
