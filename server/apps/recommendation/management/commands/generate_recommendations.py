...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--user-id", type=int, help="ID of the user to generate recommendations for (optional)")
        ...
        parser.add_argument("--count", type=int, default=10, help="Number of recommendations to generate per user")
        parser.add_argument(
            "--min-ratings",
            type=int,
            default=3,
        ...
        if user_id:
            try:
                ...
                recommendations = RecommendationService.generate_recommendations_for_user(
                    user_id=user.id, n_recommendations=count, model_id=model_id
                )
            ...
            count = RecommendationService.generate_recommendations_for_all_users(
                n_recommendations=count, model_id=model_id, min_ratings=min_ratings
            )

            self.stdout.write(
                self.style.SUCCESS(f"Generated recommendations for multiple users. Total count: {count}")
            )
