...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--ratings-per-user", type=int, default=10, help="Number of random ratings to generate per user"
        )
        parser.add_argument("--rating-range", type=str, default="1-5", help="Range of ratings (min-max)")
        parser.add_argument("--username-prefix", type=str, default="testuser", help="Prefix for test usernames")
        parser.add_argument("--password", type=str, default="password123", help="Password for test users")
        ...
        try:
            with transaction.atomic():
                ...
                for user in created_users:
                    ...
                    books_to_rate = books if len(books) <= ratings_per_user else random.sample(books, ratings_per_user)
                    ...
                    for book in books_to_rate:
                        ...
                        BookRating.objects.create(user=user, book=book, rate=rating_value)
                        ratings_created += 1
