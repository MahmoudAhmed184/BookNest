...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--min-ratings", type=int, default=5, help="Minimum number of ratings a user must have to be included"
        )
        parser.add_argument("--n-factors", type=int, default=100, help="Number of factors for SVD model")
        parser.add_argument("--knn-k", type=int, default=40, help="Number of neighbors for KNN model")
    ...
    def handle(self, *args, **options):
        ...
        try:
            model_record = RecommendationService.train_recommendation_model(
                model_type=model_type, min_ratings_per_user=min_ratings, n_factors=n_factors, knn_k=knn_k
            )

            if model_record:
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully trained {model_type.upper()} model with ID: {model_record.id}")
                )
