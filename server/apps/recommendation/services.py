...
class RecommendationService:
    ...
    @staticmethod
    def _save_model_artifact(engine: RecommendationEngine, model_type: str) -> str:
    ...
    def train_recommendation_model(
        *, model_type: str = "svd", min_ratings_per_user: int = 5, n_factors: int = 100, knn_k: int = 40
        ...
        with transaction.atomic():
            ...
            model_record = RecommendationModel(
                model_type=model_type,
                min_ratings_per_user=min_ratings_per_user,
                n_factors=n_factors,
                knn_k=knn_k,
    ...
    @staticmethod
    def load_recommendation_model(*, model_id: int | None = None) -> tuple[Any, RecommendationModel] | None:
        """
        ...
        try:
            model_record = (
                active_recommendation_model(model_id=model_id) if model_id else active_recommendation_model()
            )
            ...
            model_file_name = model_record.model_file.name
            if not model_file_name:
                logger.error("Recommendation model record has no file name")
                return None

            model_path = Path(settings.MEDIA_ROOT) / model_file_name
    ...
    @staticmethod
    def _fallback_catalog_recommendations(*, user_id: int, n_recommendations: int = 10) -> list[tuple[str, float]]:
        ...
        return [(book.isbn13, float(book.recommendation_score or 0)) for book in books]
    ...
    def generate_recommendations_for_user(
        *,
        ...
        model_data = RecommendationService.load_recommendation_model(model_id=model_id)
        if not model_data and train_if_missing:
            ...
            if model_record:
                model_data = RecommendationService.load_recommendation_model(model_id=model_record.id)
        ...
        if model_data:
            ...
            logger.info(f"No recommendation model available for user {user_id}; using catalog fallback")
            recommendations = []
    ...
    def generate_recommendations_for_all_users(
        *, n_recommendations: int = 10, model_id: int | None = None, min_ratings: int = 3
        ...
        users_with_ratings = (
            BookRating.objects.values("user")
            .annotate(rating_count=Count("rate_id"))
        ...
        logger.info(
            f"Generated recommendations for {len(user_ids)} users, total of {recommendations_count} recommendations"
        )
        return recommendations_count

    @staticmethod
    def activate_model(*, model: RecommendationModel) -> RecommendationModel:
