...
class FollowAPITests(APITestCase):
    def setUp(self) -> None:
        ...
        self.user1 = User.objects.create_user(
            username="user1_follow_api", email="user1_follow_api@example.com", password="password1"
        )
        self.profile1, _ = Profile.objects.get_or_create(user=self.user1, defaults={"bio": "Profile 1 Bio API"})

        self.user2 = User.objects.create_user(
            username="user2_follow_api", email="user2_follow_api@example.com", password="password2"
        )
        self.profile2, _ = Profile.objects.get_or_create(user=self.user2, defaults={"bio": "Profile 2 Bio API"})

        self.user3 = User.objects.create_user(
            username="user3_follow_api", email="user3_follow_api@example.com", password="password3"
        )
    ...
    def test_unfollow_user_authenticated(self) -> None:
        self.client.force_authenticate(user=self.user1)
    ...
    def test_unfollow_user_not_following(self) -> None:
        self.client.force_authenticate(user=self.user1)
        response = self.client.delete(self.unfollow_url(999999))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_get_my_followers_list_authenticated(self) -> None:
    ...
    def test_get_my_following_list_authenticated(self) -> None:
        Follow.objects.create(follower=self.profile1, followed=self.profile2)
        self.client.force_authenticate(user=self.user1)
    ...
    def test_get_user_followers_list_public(self) -> None:
    ...
    def test_get_user_following_list_public(self) -> None:
        Follow.objects.create(follower=self.profile1, followed=self.profile2)
        Follow.objects.create(follower=self.profile1, followed=self.profile3)
        self.client.force_authenticate(user=self.user1)
